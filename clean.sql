...
IF OBJECT_ID('tempdb..#partitioned_columns', 'U') IS NOT NULL DROP TABLE #partitioned_columns;
...
IF OBJECT_ID('tempdb..#remaning_columns', 'U') IS NOT NULL DROP TABLE #remaning_columns;
...
EXEC sp_rename 'customers.Order_Id', 'customer_id', 'COLUMN';
;
...
WHERE EXISTS (
    ...
    WHERE TABLE_NAME = 'orders_demographic'
        ...
        AND (
			...
			CHARINDEX(@char_error, Order_Region) > 0 OR
			CHARINDEX(@char_error, Order_Country) > 0 OR
			CHARINDEX(@char_error, Order_State) > 0 OR
			CHARINDEX(@char_error, Order_City) > 0 OR
			CHARINDEX(@char_error, Order_Zipcode) > 0 OR
